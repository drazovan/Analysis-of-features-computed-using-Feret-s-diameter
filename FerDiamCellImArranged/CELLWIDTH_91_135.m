function cellWidth_91_135=CELLWIDTH_91_135(fi,I)
...
for i=1:round(w/2)
    for j=1:round(h/2)
        ...
        A=Af(i,j,:);
       mask = (A>0 & A<=1);
            mask1 = A<cot(angle)/2; %trougao 
            
                P1P2(mask & mask1)=sqrt(max(0,(2*(A(mask & mask1)))./(cos(fi)*sin(angle)))); DP1(mask & mask1)=P1P2(mask & mask1)*cos(angle);  %DP2=P1P2*sin(fi); %proveriti P1P2
                P1(mask & mask1)=[i*fill(mask & mask1)',(j-1+DP1(mask & mask1))']*[COS;SIN];                                               %P2=[i-DP2,j]*[cos(fi),sin(fi)]';
            
            mask2 = (A>=cot(angle)/2 & A<=1-cot(angle)/2); %trapez
                
                DP2(mask & mask2)=cot(angle); AP1(mask & mask2)=(2*A(mask & mask2)-cot(angle))/2;                                              %BP2=(2*Af(i,j,l)+tan(fi))/2; 
                P1(mask & mask2)=[(i-1+AP1(mask & mask2))',((j-1)*fill(mask & mask2)-1)']*[COS;SIN];                                           %P2=[i-BP2,j]*[cos(fi),sin(fi)]';
            
            mask3 = (A>1-cot(angle)/2); %dopunjen trougao
...
P=zeros(w,h,368); 
 ...
 cellWidth_91_135=projMax-projMin;
